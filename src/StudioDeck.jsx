...
function StudioFace({scene,running,reduce}){
  ...
  return <><div className="studio-visual"><div className={`studio-canvas ${architecture?'studio-workflow':'studio-recording'}`} data-time={time.toFixed(2)} data-running={enabled}>
    {architecture?<iframe ref={frame} src={scene.media.src} title="视频生产 Agent 工作流编排动画" onLoad={()=>setFrameReady(true)} tabIndex={-1}/>:failed||!source?<img src={scene.media.poster} alt="视频工作台演示封面"/>:<video data-analytics-project="studio" ref={video} src={source||undefined} poster={scene.media.poster} muted playsInline loop preload="metadata" onPlaying={()=>{setPlaying(true);setWaiting(false);}} onPause={()=>{setPlaying(false);setWaiting(false);}} onWaiting={()=>{setPlaying(false);setWaiting(true);}} onSeeking={()=>{setPlaying(false);setWaiting(true);}} onTimeUpdate={e=>{if(!dragging.current)setTime(e.currentTarget.currentTime);}} onError={e=>{if(source?.startsWith('blob:')&&e.currentTarget.error?.code===4){useCachedFile();return;}setFailed(true);setPaused(true);setPlaying(false);setWaiting(false);}}/>}
