...
function StudioFace({scene,running,reduce}){
  ...
  return <><div className="studio-visual"><div className={`studio-canvas ${architecture?'studio-workflow':'studio-recording'}`} data-time={time.toFixed(2)} data-running={enabled}>
    ...
    {!architecture&&<aside className="studio-copy"><span className="feature-label mono">{scene.label}</span><h2>{scene.title.map(line=><span key={line}>{line}</span>)}</h2><p>{scene.description}</p><ul>{scene.points.map((point,i)=><li key={point}><span>0{i+1}</span>{point}</li>)}</ul><div className="studio-face-number">01<span>CREATOR<br/>STUDIO</span></div></aside>}</>;
}
